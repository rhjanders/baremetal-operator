...
func TestServiceAnnotationWithServicing(t *testing.T) {
	...
	hup := &metal3api.HostUpdatePolicy{
		...
		Spec: metal3api.HostUpdatePolicySpec{
			FirmwareSettings: metal3api.HostUpdatePolicyOnService,
			FirmwareUpdates:  metal3api.HostUpdatePolicyOnService,
		},
	}
...
// TestServiceAnnotationWithOnRebootPolicy tests that service annotation
// also works with the legacy onReboot HUP policy value.
func TestServiceAnnotationWithOnRebootPolicy(t *testing.T) {
	host := newDefaultHost(t)
	host.Annotations = make(map[string]string)
	host.Annotations[metal3api.ServiceAnnotationPrefix] = ""
	host.Status.PoweredOn = true
...
func TestServiceAnnotationWithoutHUP(t *testing.T) {
	...
	assert.True(t, host.Status.PoweredOn)
	assert.NotEqual(t, metal3api.OperationalStatusServicing, host.Status.OperationalStatus)
}
